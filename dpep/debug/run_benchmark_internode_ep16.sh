...
# Benchmark: Inter-node EP16 (2 nodes, 8 GPUs each via EFA)
# Tests EFA communication at larger scale

NODE1_IP=172.25.105.98
NODE2_IP=172.25.105.130
MASTER_ADDR=$NODE1_IP

echo "=== Running Inter-node EP16 Benchmark (EFA) ==="
echo "Node 1: $NODE1_IP (rank 0-7)"
echo "Node 2: $NODE2_IP (rank 8-15)"
...
python -m tests.bench_all_to_all \
    ...
    --in-dtype bfloat16 \
    --out-dtype bfloat16 \
    2>&1 | tee /tmp/benchmark_internode_ep16_node2.log
EOF
...
python -m tests.bench_all_to_all \
    ...
    --in-dtype bfloat16 \
    --out-dtype bfloat16 \
    2>&1 | tee /tmp/benchmark_internode_ep16_node1.log
...
scp -P 2222 $NODE1_IP:/tmp/pplx_workspace/pplx-kernels/data/*.tsv ./results_internode_ep16.tsv 2>/dev/null || true
echo "Results saved to: ./results_internode_ep16.tsv"
